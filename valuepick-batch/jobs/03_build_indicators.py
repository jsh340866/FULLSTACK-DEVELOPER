...
def join_dividend_yield(df: DataFrame, dividends: DataFrame) -> DataFrame:
    ...
    ).withColumn("dividend_amount", parse_amount("thstrm")) \
     .select("stock_code", F.col("year").alias("dividend_year"), "dividend_amount")
...
def join_momentum(df: DataFrame, prices_1m: DataFrame, prices_12m: DataFrame) -> DataFrame:
    ...
    p1m = prices_1m.select("stock_code", F.col("close_price").alias("price_1m_ago"),
                            F.col("listed_share_count").alias("share_count_1m_ago"))
    p12m = prices_12m.select("stock_code", F.col("close_price").alias("price_12m_ago"),
...
def main():
    ...
    result = df.select(
        "stock_code", "year", "fs_div", "currency",
